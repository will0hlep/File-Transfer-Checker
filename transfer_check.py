...
missing_files = [] #list of missing folders
origin_files = list(os.walk(ORIGIN_PATH)) #retrieve a list of all files and folders in the origin folder
...
for origin_dir, _, origin_files in origin_files: #loop through all directories listed in origin_files
    for origin_file in origin_files: #loop through all files in the current directory
        ...
        if not os.path.exists(destination_file_path): #check if the file exists in the destination folder
            missing_files.append(origin_file_path) #if the file does not exist, add it to the list of missing files

        files_checked += 1 #increment files_checked counter

if missing_files != []: #checks if any files are missing
    ...
    with open(".\\report.txt", 'w', encoding="utf-8") as output: #open a text file to write the list of missing files
        for row in missing_files: #loop through the list of missing files
            output.write(str(row) + '\n') #write each missing file path to the text file, followed by a newline character
